...
export class SportRepository implements ISportRepository {
  ...
  findSportById(id: string): Promise<SportEntity | null> { return this.sportRepo.findOne({ where: { id } }); }
  createSport(data: Partial<SportEntity>): Promise<SportEntity> { return this.sportRepo.save(this.sportRepo.create(data)); }
  ...
  async deleteSport(id: string): Promise<void> { await this.sportRepo.delete(id); }

  findCategoriesBySport(sportId: string): Promise<SportCategoryEntity[]> {
    return this.catRepo.find({ where: { sportId }, order: { order: 'ASC' } });
  }
  findCategoryById(id: string): Promise<SportCategoryEntity | null> { return this.catRepo.findOne({ where: { id } }); }
  createCategory(data: Partial<SportCategoryEntity>): Promise<SportCategoryEntity> { return this.catRepo.save(this.catRepo.create(data)); }
  ...
  async deleteCategory(id: string): Promise<void> { await this.catRepo.delete(id); }
  ...
  findGroupById(id: string): Promise<SportCategoryGroupEntity | null> { return this.groupRepo.findOne({ where: { id } }); }
  createGroup(data: Partial<SportCategoryGroupEntity>): Promise<SportCategoryGroupEntity> { return this.groupRepo.save(this.groupRepo.create(data)); }
  createGroups(data: Partial<SportCategoryGroupEntity>[]): Promise<SportCategoryGroupEntity[]> { return this.groupRepo.save(data.map(d => this.groupRepo.create(d))); }
  ...
  async deleteGroup(id: string): Promise<void> { await this.groupRepo.delete(id); }

  findAssignedTeams(sportId: string, sportCategoryId?: string | null): Promise<SportCategoryTeamEntity[]> {
    const where: any = { sportId };
    if (sportCategoryId === null) where.sportCategoryId = IsNull();
    else if (sportCategoryId) where.sportCategoryId = sportCategoryId;
    return this.teamRepo.find({ where, relations: ['raffleTeam'] });
  }
  assignTeam(data: Partial<SportCategoryTeamEntity>): Promise<SportCategoryTeamEntity> { return this.teamRepo.save(this.teamRepo.create(data)); }
  async removeTeamAssignment(id: string): Promise<void> { await this.teamRepo.delete(id); }
}